<h1 align="center">🚀 Portfólio Profissional – Paulo Weslley (PW)</h1>

<p align="center">
  <img src="https://img.shields.io/badge/Status-Em%20Desenvolvimento-blue?style=for-the-badge">
  <img src="https://img.shields.io/badge/Focus-Analista%20de%20Dados%20%7C%20Back--End%20Python-orange?style=for-the-badge">
  <img src="https://img.shields.io/badge/Version-1.3-lightgrey?style=for-the-badge">
</p>

<p align="center">
  <img src="https://github.com/weslley87exe/PW_Portfolio/blob/main/assets/banner_pw.png" alt="Banner PW" width="80%">
</p>

---

## 🧠 Sobre mim
Sou **Paulo Weslley (PW)**, estudante de **Análise e Desenvolvimento de Sistemas** e apaixonado por tecnologia, dados e desenvolvimento.  
Atualmente focado em **Análise de Dados** e **Back-End com Python**, construindo projetos que unem **lógica, automação e visualização de dados**.

> “Transformar dados em decisões é mais do que um trabalho — é um superpoder.” ⚡

---

## 🧩 Tecnologias & Ferramentas

<div align="center">

<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/python/python-original.svg" width="45" height="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/sqlite/sqlite-original.svg" width="45" height="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" width="45" height="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/css3/css3-original.svg" width="45" height="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/git/git-original.svg" width="45" height="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/github/github-original.svg" width="45" height="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/vscode/vscode-original.svg" width="45" height="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pandas/pandas-original.svg" width="45" height="45"/>
<img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/numpy/numpy-original.svg" width="45" height="45"/>

</div>

---

## 📈 Projetos em Destaque

---

### 🐍 **Sistema de Cadastro de Alunos (Python + FastAPI)**
Sistema completo desenvolvido em **Python + FastAPI**, com:

- CRUD completo  
- Banco SQLite  
- Templates HTML com Jinja2  
- Validação de notas com vírgula  
- Deploy automático via Railway  

🔗 **Acessar sistema:** https://sistema-alunos-pwdev.up.railway.app  
📦 **Código:** https://github.com/weslley87exe/PW_Portfolio/tree/main/Python_projects/sistema_alunos

---

### 📊 **Dashboard de Vendas – Análise Completa (Power BI)**
Dashboard avançado cobrindo:

- KPIs essenciais  
- Filtros dinâmicos  
- DAX avançado  
- Análise 2015–2024  
- Visual refinado e responsivo  

🖼️ **Visual do Dashboard:**  
👉 [Ver Print em Alta Qualidade](PowerBI_Dashboards/Dashboard-vendas-analise-avancada/Dashboard/Dashboard%20de%20Vendas%20—%20An%C3%A1lise%20Avan%C3%A7ada%20(2015–2024).png)

📦 **Arquivos do Projeto:**  
👉 [Abrir Pasta no GitHub](PowerBI_Dashboards/Dashboard-vendas-analise-avancada)

---

### 📊 **Dashboard Vendas Anual 2023 (Power BI)**
Dashboard rápido e objetivo para análise anual.

🖼️ **Visual do Dashboard:**  
👉 [Ver Print Anual 2023](PowerBI_Dashboards/Vendas_Anual/Vendas_Anual_2023_PW.PNG)

---

### 💡 **Mini Projeto de Análise de Dados (Em desenvolvimento)**

- Pandas  
- Matplotlib  
- Relatórios automáticos  
- Insights reais  
- CSV + Python  

---

## 🗂️ Sumário dos Projetos

| 💻 Projeto | 🧠 Tecnologias | 🔗 Acesso |
|-----------|----------------|-----------|
| Sistema de Cadastro (FastAPI) | Python, SQLite | [Acessar](https://github.com/weslley87exe/PW_Portfolio/tree/main/Python_projects/sistema_alunos) |
| Dashboard Completo de Vendas | Power BI, DAX, SQL | [Acessar](https://github.com/weslley87exe/PW_Portfolio/tree/main/PowerBI_Dashboards/Dashboard-vendas-analise-avancada) |
| Dashboard Vendas 2023 | Power BI | [Acessar](https://github.com/weslley87exe/PW_Portfolio/tree/main/PowerBI_Dashboards/Vendas_Anual) |
| Mini Análise de Dados | Python, Pandas | [Acessar](https://github.com/weslley87exe/PW_Portfolio/tree/main/Python_projects/analise_dados_vendas) |

---

### 📬 **Meus Contatos**
📧 **Email:** pauloweslleypw8@gmail.com  
💼 **LinkedIn:**  
👉 [Acessar Perfil](https://www.linkedin.com/in/paulo-wesley-mesquita-oliveira-841b0b377)  
🐙 **GitHub:**  
👉 [Meu Repositório](https://github.com/weslley87exe)  
🖥️ **Portfólio Online:**  
👉 [Ver Portfólio Completo](https://weslley87exe.github.io/PW_Portfolio)


---

# 🌍 English Version

<h3 align="center">🚀 Professional Portfolio – Paulo Weslley (PW)</h3>

## 🧠 About Me
I'm a **Systems Analysis and Development** student passionate about **data, development, and automation**.

Currently focused on:

- Data Analysis  
- Python Back-End  
- Dashboards  
- Real-world problem solving  

---

## 🧩 Tech Stack

(Same icons as above)

---

## 📈 Featured Projects

### Student Registration System (Python + FastAPI)
Live API  
SQLite  
Jinja2  
Validation  
Deploy on Railway  

🔗 App: https://sistema-alunos-pwdev.up.railway.app  
🔗 Code: https://github.com/weslley87exe/PW_Portfolio/tree/main/Python_projects/sistema_alunos

---

### Power BI – Full Sales Dashboard
Advanced analysis from 2015–2024.

---

### Sales Annual Dashboard 2023

---

## 🌐 Contact

Same links as above.
